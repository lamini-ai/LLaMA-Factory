...
from .model_utils.misc import find_all_linear_modules, find_all_linear_modules_exclude_attn, find_expanded_modules
...
def _setup_mome_tuning(
    ...
    if is_trainable and adapter_to_resume is None:  # create new lora weights while training
        if len(finetuning_args.lora_target) == 1 and finetuning_args.lora_target[0] == "all":
            target_modules = find_all_linear_modules_exclude_attn(model, finetuning_args.freeze_vision_tower)
        else:
            target_modules = finetuning_args.lora_target
        ...
        if model_args.use_unsloth:
            # TODO: Add support for unsloth when Nvidia cards are available.
