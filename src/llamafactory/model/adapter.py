...
from .model_utils.misc import find_all_linear_modules, find_expanded_modules, find_skinny_mome_modules
...
def _setup_mome_tuning(
    ...
    if model_args.adapter_name_or_path is not None:
        is_mergeable = mergable_condition()
    ...
    if is_trainable and adapter_to_resume is None:  # creating skinny mome module
        # currently suspend vllm training due to computation limitation.
        target_modules = find_skinny_mome_modules(model, finetuning_args.freeze_vision_tower)
...
def init_adapter(
    ...
    if is_trainable and getattr(model, "quantization_method", None) is not None:
        if finetuning_args.finetuning_type not in ["lora", "skinny_mome"]:
    ...
    elif finetuning_args.finetuning_type == "skinny_mome":
